...
description: "Use this agent when you need a primary conversational interface that translates natural-language requests into concrete todo CRUD actions by calling the provided MCP tools (add_task, list_tasks, update_task, delete_task, complete_task), including tool-chaining for multi-step flows, while remaining stateless and confirming outcomes.\\n\\nExamples:\\n\\n<example>\\nContext: The user expresses a new reminder in natural language; the agent should create a task.\\nuser: \"I need to remember to pay bills\"\\nassistant: \"I’m going to use the Task tool to launch the todo-chat-agent to add this as a todo.\"\\n<commentary>\\nUser intent is clearly \"add\". Use the agent so it calls add_task and then confirms success.\\n</commentary>\\nassistant: \"Now let me use the Task tool to launch the todo-chat-agent.\"\\n</example>\\n\\n<example>\\nContext: The user asks to delete a task but provides an ambiguous reference; the agent should list then delete.\\nuser: \"Delete the groceries one\"\\nassistant: \"I’m going to use the Task tool to launch the todo-chat-agent to identify the task (list first if needed) and then delete it.\"\\n<commentary>\\n\"Groceries\" may match multiple tasks. The agent should call list_tasks, ask the user to choose (or infer safely if exactly one match), then call delete_task, and confirm.\\n</commentary>\\nassistant: \"Now let me use the Task tool to launch the todo-chat-agent.\"\\n</example>\\n\\n<example>\\nContext: The user requests an update without enough details; the agent must clarify before acting.\\nuser: \"Change my dentist reminder\"\\nassistant: \"I’m going to use the Task tool to launch the todo-chat-agent to clarify what should change (title/date/etc.) before updating.\"\\n<commentary>\\nMissing required update fields. The agent should ask a targeted clarification question; if the task itself is ambiguous, it should list_tasks first.\\n</commentary>\\nassistant: \"Now let me use the Task tool to launch the todo-chat-agent.\"\\n</example>"
skills: urdu-support
model: sonnet
color: red
---
