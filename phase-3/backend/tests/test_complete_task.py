...
from mcp_server.tools import handle_complete_task
...
@pytest.mark.asyncio
async def test_handle_complete_task_success():
    ...
    # Create mock task (not completed yet)
    mock_task = Task(
        id=task_id,
        user_id=user_id,
    ...
    with patch('mcp_server.tools.get_session') as mock_get_session:
        ...
        with patch('mcp_server.tools.User') as mock_user_class:
            ...
            # Call the handler
            result = await handle_complete_task(arguments, user_id)

            # Verify the response
            assert len(result) == 1
            assert result[0].type == "text"
            assert "Successfully marked task" in result[0].text
            assert "as completed" in result[0].text
            assert "Task to Complete" in result[0].text


@pytest.mark.asyncio
async def test_handle_complete_task_already_completed():
    ...
    # Create mock task (already completed)
    mock_task = Task(
        id=task_id,
        user_id=user_id,
    ...
    with patch('mcp_server.tools.get_session') as mock_get_session:
        ...
        with patch('mcp_server.tools.User') as mock_user_class:
            ...
            assert len(result) == 1
            assert result[0].type == "text"
            assert "is already completed" in result[0].text
            assert "Already Completed Task" in result[0].text


@pytest.mark.asyncio
async def test_handle_complete_task_not_found():
...
@pytest.mark.asyncio
async def test_handle_complete_task_invalid_task_id():
    """Test completing with an invalid task ID format"""
    # Mock user ID
    user_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_handle_complete_task_wrong_user():
