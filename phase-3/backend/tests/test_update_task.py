...
from mcp_server.tools import handle_update_task
...
async def test_handle_update_task_success():
    ...
    with patch('mcp_server.tools.get_session') as mock_get_session:
        ...
        with patch('mcp_server.tools.User') as mock_user_class:
            ...
            # Call the handler
            result = await handle_update_task(arguments, user_id)

            # Verify the response
            assert len(result) == 1
            assert result[0].type == "text"
            assert "Successfully updated task" in result[0].text
            assert "Updated Task Title" in result[0].text
...
@pytest.mark.asyncio
async def test_handle_update_task_invalid_task_id():
    """Test updating with an invalid task ID format"""
    # Mock user ID
    user_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_handle_update_task_wrong_user():
