...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    try {
      const response = await apiClient.login(email, password, true); // Show toast on error
      ...
      // Error is already shown as toast by the API client
      throw error;
    }
  };
  ...
  const register = async (email: string, password: string, name: string) => {
    try {
      const response = await apiClient.register(email, password, name, true); // Show toast on error
