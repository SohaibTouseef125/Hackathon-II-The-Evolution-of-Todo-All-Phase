...
name: mcp-task-tools
description: |
  Use this agent when you need to expose or invoke stateless MCP tools that perform
  validated, database-backed CRUD operations for tasks and must return structured
  JSON only (no natural language output).
  ...
  This includes:
    ...
    - Returning machine-readable error objects

  When to use this agent:
    - Any task-related operation that must be executed via MCP tools
    - Situations where output must be strictly JSON
    - Database-backed state changes or reads with no conversational text

  Examples:
    - Context: User requests creation of a new task as part of a workflow.
      ...
      Commentary: |
        Since the task must be created via stateless MCP tools and the response
        must be machine-readable JSON only, the mcp-task-tools agent is required.

    - Context: User requests a list of current tasks.
      User: "List all my incomplete tasks"
      Assistant: "I'm going to use the Task tool to launch the mcp-task-tools agent to call list_tasks with a status filter and return JSON only."
      Commentary: |
        Listing tasks is a database read operation that must return structured
        JSON with no conversational text.

    - Context: User marks a task as completed.
      ...
      Commentary: |
        This is a persisted state change and must be executed via MCP tools
        with strictly structured JSON output.
